...
description: 큰 Figma 프레임 하나(MCP 링크)를 여러 sub-agent로 깊게 탐색하고(truncate되면 자식 nodeId로 재귀), 어느 코드베이스(예: front/admin)에 속하는지 스스로 판단해 우리 코드베이스의 실제 컴포넌트로 매핑해서, 해당 화면을 "우리 코드 맥락"으로 이해한 구조화 결과를 반환하는 헬퍼 스킬. **상위 스킬이 호출하거나 /figma-depth-find로 명시 호출될 때만 발동한다. 일반적인 Figma 언급·URL 공유만으로는 절대 자동 발동하지 않는다.**
---
...
- **단독 호출 안 됨이 기본.** 보통 상위 스킬(예: 시안→구현, 신규 페이지 작업 등)이 "이 Figma를 우리 컴포넌트 맥락으로 이해해와" 라고 위임할 때 불린다.
- 자동 발동 금지. Figma URL을 공유했다거나 "이 디자인 봐줘" 같은 일반 표현엔 **반응하지 않는다.** 오직:
  - 상위 스킬이 명시적으로 이 스킬을 호출, 또는
  - 사용자가 `/figma-depth-find <URL>` 로 직접 호출
  할 때만 동작한다.
- 산출물은 **콘솔 출력 + 상위 호출자에게 반환하는 구조화 데이터**다. 파일 저장은 호출자가 명시 요청할 때만.
